...
<head>    <title>Financial Analyzer</title>
    ...
    <style>
        ...
        .company-card { background: #1e293b; padding: 16px; border-radius: 10px; margin-bottom: 12px; transition: all 0.3s; }
        .company-card:hover { background: #334155; }
        .btn-action { padding: 6px 12px; border-radius: 6px; text-decoration: none; font-size: 13px; margin-right: 6px; border: none; cursor: pointer; transition: all 0.2s; display: inline-block; }
        .btn-edit { background: #f59e0b; color: white; } .btn-edit:hover { background: #d97706; }
        .btn-delete { background: #ef4444; color: white; } .btn-delete:hover { background: #dc2626; }
        .btn-input { background: #10b981; color: white; } .btn-input:hover { background: #059669; }
        .btn-analysis { background: #06b6d4; color: white; } .btn-analysis:hover { background: #0891b2; }
        .modal-overlay { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); z-index: 1000; justify-content: center; align-items: center; }
        .modal-box { background: #1e293b; padding: 30px; border-radius: 15px; width: 90%; max-width: 500px; color: white; }
        .toast { position: fixed; bottom: 20px; right: 20px; padding: 12px 20px; border-radius: 8px; color: white; z-index: 2000; display: none; }
        ...
        for c in companies:
            ...
            <div class="company-card" id="company-{c['_id']}">
                ...
                <div style="margin-top:8px;">
                    <a href="/input-data/{c['_id']}" class="btn-action btn-input"><i class="bi bi-pencil-square"></i> Input Data</a>
                    <a href="/analysis/{c['_id']}" class="btn-action btn-analysis"><i class="bi bi-graph-up"></i> View Analysis</a>
                </div>
            </div>"""
    else:
...
async def input_data_page(request: Request, company_id: str):
    """Form to input financial data + DCF settings"""
    try:
    ...
    <div class="container">
        ...
        <form id="financialForm">
            ...
            <div class="form-section">
                ...
                <div class="row">
                    <div class="col-md-3"><label>Report Type *</label><select class="form-control" id="reportType" required><option value="annual">Annual</option><option value="quarterly">Quarterly</option></select></div>
                    <div class="col-md-3"><label>Year *</label><input type="number" class="form-control" id="year" placeholder="2024" required></div>
            ...
            <div class="form-section" id="dcfSection">
                ...
                <div class="row mb-3">
                    <div class="col-md-3"><label>Projection Years</label><select class="form-control" id="dcfYears"><option value="5">5 Years</option><option value="7">7 Years</option><option value="10">10 Years</option></select></div>
                    <div class="col-md-3"><label>WACC (%)</label><input type="number" step="0.1" class="form-control" id="dcfWacc" value="15.0" min="5" max="30"></div>
                    <div class="col-md-3"><label>Terminal Growth (%)</label><input type="number" step="0.1" class="form-control" id="dcfTermGrowth" value="3.0" min="0" max="6"></div>
                    <div class="col-md-3"><label>Margin of Safety (%)</label><input type="number" step="1" class="form-control" id="dcfMos" value="20" min="0" max="50"></div>
                </div>
    ...
    <script>
        ...
        function renderGrowthInputs(years = 5) {{
            ...
            for(let i = 0; i < years; i++) {{
                container.innerHTML += `<div style="min-width:75px;"><label style="font-size:11px;">Y${{i+1}}</label><input type="number" step="0.1" class="form-control form-control-sm growth-rate" value="${{defaults[i] || 3}}" min="0" max="50" data-year="${{i+1}}"></div>`;
            }}
        }}
        ...
        document.getElementById('financialForm').addEventListener('submit', async function(e) {{
            ...
            var data = {{
                ...
                total_assets: parseFloat(document.getElementById('totalAssets').value) || 0, total_liabilities: parseFloat(document.getElementById('totalLiabilities').value) || 0,
                shareholders_equity: parseFloat(document.getElementById('shareholdersEquity').value) || 0, total_debt: parseFloat(document.getElementById('totalDebt').value) || 0,
                cash_and_equivalents: parseFloat(document.getElementById('cash').value) || 0, current_assets: parseFloat(document.getElementById('currentAssets').value) || 0,
                current_liabilities: parseFloat(document.getElementById('currentLiabilities').value) || 0, revenue: parseFloat(document.getElementById('revenue').value) || 0,
                gross_profit: parseFloat(document.getElementById('grossProfit').value) || 0, operating_income: parseFloat(document.getElementById('operatingIncome').value) || 0,
            ...
            try {{
                ...
                if (response.ok) {{ window.location.href = '/result/' + result.financial_id; }}
                else {{ alert('Error: ' + (result.detail || 'Something went wrong')); btn.disabled = false; btn.innerHTML = '📊 Analyze & Save'; }}
            }} catch (error) {{ alert('Error: ' + error.message); btn.disabled = false; btn.innerHTML = '📊 Analyze & Save'; }}
        }});
    </script>
</body>
</html>"""
...
async def create_company(company: CompanyCreate):
    ...
    if existing: raise HTTPException(400, "Company with this ticker already exists")
    doc = {"name": company.name, "ticker": company.ticker.upper(), "sector": company.sector, "sub_sector": company.sub_sector, "is_active": True, "created_at": datetime.utcnow()}
...
async def update_company(company_id: str, company: CompanyCreate):
    ...
    duplicate = await db.companies.find_one({"ticker": company.ticker.upper(), "_id": {"$ne": obj_id}})
    if duplicate: raise HTTPException(400, "Another company with this ticker already exists")
    await db.companies.update_one({"_id": obj_id}, {"$set": {"name": company.name, "ticker": company.ticker.upper(), "sector": company.sector, "sub_sector": company.sub_sector, "updated_at": datetime.utcnow()}})
...
async def create_financial_data(data: FinancialDataCreate):
    ...
    analysis_doc = {
        ...
        "year": data.year, "quarter": data.quarter, "overall_score": health_analysis["overall_score"],
        "overall_color": health_analysis["overall_color"], "overall_color_name": health_analysis["overall_color_name"],
        "metrics": health_analysis["metrics"], "ratios": health_analysis["ratios"],
