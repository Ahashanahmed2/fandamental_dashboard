...
class DCFValuation:
    ...
    def calculate_terminal_value(
        ...
        wacc: float
    ) -> float:        """Gordon Growth Model দিয়ে টার্মিনাল ভ্যালু"""
    ...
    def run_dcf(
        ...
        fcf_margin = financial_data.get("fcf_margin", current_fcf / revenue if revenue > 0 else 0.05)
        
        shares_outstanding = financial_data.get("shares_outstanding", 1)
        net_debt = (financial_data.get("total_debt", 0) - 
        ...
        if dcf_inputs.beta and dcf_inputs.risk_free_rate and dcf_inputs.market_risk_premium:
            ...
            debt_value = financial_data.get("total_debt", 0)
            wacc = DCFValuation.calculate_wacc(
                equity_value, debt_value, cost_of_equity, 
                dcf_inputs.wacc * 0.7,  # cost_of_debt estimate                dcf_inputs.tax_rate
            )
        
        # ৩. FCF প্রজেকশন
        fcf_projections = DCFValuation.project_fcf(
            current_fcf, 
            dcf_inputs.revenue_growth_rates[:dcf_inputs.projection_years],
            revenue,
            fcf_margin
        )
        ...
        return {
            # ইনপুট সামারি
            "inputs": {
                "wacc": round(wacc * 100, 2),                "terminal_growth": round(dcf_inputs.terminal_growth_rate * 100, 2),
    ...
    def sensitivity_analysis(
        ...
        for wacc in wacc_range:
            ...
            for tg in growth_range:
                ...
                equity = pv_tv - net_debt                value_per_share = equity / shares if shares > 0 else 0
